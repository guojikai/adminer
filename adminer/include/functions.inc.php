...
function where($where, $fields = array()) {
	...
	foreach ((array) $where["where"] as $key => $val) {
		...
		; //! enum and set
		if ($jush == "sql" && $fields[$key] != null && preg_match('~char|text~', $fields[$key]["type"]) && preg_match("~[^ -@]~", $val)) { // not just [a-z] to catch non-ASCII characters
...
<?php
	if (!$fields) {
		...
		foreach ($fields as $name => $field) {
			...
			$default = $_GET["set"] != null ? $_GET["set"][bracket_escape($name)] : null;
			if ($default === null) {
