		...
		if (!$rows) {
			...
			foreach ($rows[0] as $key => $val) {
				if (!isset($unselected[$key])) {
					$val = $_GET["columns"] ? $_GET["columns"][key($select)] : null;
			...
			foreach ($adminer->rowDescriptions($rows, $foreign_keys) as $n => $row) {
				...
				foreach ($row as $key => $val) {
					if (isset($names[$key])) {
						...
						$unique_idf_val = $_POST["val"] ? $_POST["val"][$unique_idf] : null;
						$be_value = $unique_idf_val ? $unique_idf_val[bracket_escape($key)] : null;
						$value = $be_value;
